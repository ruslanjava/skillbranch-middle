...
abstract class BaseFragment<T : BaseViewModel<out IViewModelState>> : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // bind default values if viewmodel not loaded
        if (binding?.isInflated == false) {
    ...
    override fun onPrepareOptionsMenu(menu: Menu) {
        if (root.toolbarBuilder.items.isNotEmpty()) {
            for ((index, menuHolder) in root.toolbarBuilder.items.withIndex()) {
                val item: MenuItem = menu.add(0, menuHolder.menuId, index, menuHolder.title)
    ...
    open fun renderLoading(loadingType: Loading) {
        root.renderLoading(loadingType)
    }
